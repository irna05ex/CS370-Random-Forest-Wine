...
public class GUISwitch implements ActionListener{
	...
	GUISwitch(RandomForest forest, float [] def) {
		...
		popup.setTitle("Wine Quality Tester");
		...
		JLabel Bl = new JLabel("Color/Hue");
		...
		JLabel FA = new JLabel("Fixed Acidity (g(tartaric acid)/liter):");
		...
		JLabel VA = new JLabel("Volatile Acidity (g(acetic acid)/liter):");
		...
		JLabel CA = new JLabel("Citric Acid (g/liter):");
		...
		JLabel RS = new JLabel("Residual Sugar (g/liter):");
		...
		JLabel Ch = new JLabel("Chlorides (g(sodium chloride)/liter):");
		...
		JLabel FSD = new JLabel("Free Sulfur Dioxide (mg/liter):");
		...
		JLabel TSD = new JLabel("Total Sulfur Dioxide (mg/liter):");
		...
		JLabel Dn = new JLabel("Density (g/liter):");
		...
		JLabel Su = new JLabel("Sulphates (g(potassium sulphate/liter)):");
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if(nameB.equals("Test")) {//button from inputGUI to test wine and display outputGUI
			...
			if(wineIn.getAlcohol() < 0 || wineIn.getChlorides() < 0 || wineIn.getDensity() < 0 || wineIn.getFixedAcidity() < 0 ||
				wineIn.getVolatileAcidity() < 0	|| wineIn.getFreeSulfur() < 0 || wineIn.getTotalSulfur() < 0 || wineIn.getCitricAcid() < 0
				|| wineIn.getSulphates() < 0 || wineIn.getResidualSugar() < 0 || wineIn.getpH() < 0) {
		...
		else if(nameB.equals("Try another Wine")){//button on output GUI return to inputGUI and input new wine
			popup.remove(outGUI);
			popup.add(inGUI);
			popup.revalidate();
			popup.setSize(700, 500);
		}
		else if(nameB.equals("Find similar Wine")) {//button on output GUI to find 1 similar wine and display its values
			wine simli = wineClassifier.findWine(wineIn);
			if(simli == null){//a similar wine not found
				get1.setText("NA");
				get2.setText("NA");
				get3.setText("NA");
				get4.setText("NA");
		...
		else if(nameB.equals("Input new Wine")){//button on similar wine GUI to return to input GUI
