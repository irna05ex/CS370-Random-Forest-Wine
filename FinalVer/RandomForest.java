...
public class RandomForest {
	...
	RandomForest(int tree, int depth, String at[]){
		treeNum = tree;
		maxDepth = depth;
		attributes = at;//Color should be the first attribute listed
		forest = new decisionTree[treeNum];
	}

	public wine[] returnSet(){//return data set loaded from files
		return dataSet;
	}
	
	//red should be file for red wine, white should be file for white wine
	public void loadData(Scanner red, Scanner white) {//given data set is divided into 2 files (1 for red, 1 for white)
		...
		while(red.hasNextLine()) {
			...
			atr = s.split(";");
			in = new wine();
			//attributes are in a specific order
			//get attributes based on that order
			...
			if(q > 6)//normalize data set quality
				in.setQuality(1);//good quality
			else
				in.setQuality(0);//bad quality
			
			in.setType("red");//from red wine data set
			dataSet[i] = in;
			i++;
		}
		...
		while(white.hasNextLine()) {
			...
			in.setType("white");//from white wine data set
			dataSet[i] = in;
			i++;
		}
	...
	public void buildForest(){//build Random Forest
		...
		for(int i = 0; i<treeNum; i++) {//create each tree
			...
			forest[i].buildTree();//build newly created decision tree
		}
	}
	
	//test wine for classification
	public String test(wine w) {
		String answer;
		int class1 = 0;//good
		int class2 = 0;//bad
		for(int i = 0; i<treeNum; i++) {//get each trees vote
		...
		if(class1 > class2)//return majority vote
			return "good quality";
		return "bad quality";
	}
	...
	//read file to reconstruct Random Forest
	public void readForest(Scanner read) {
		String r = read.nextLine();//skip title line
		r = read.nextLine();
		treeNum = Integer.parseInt(r);//get number of trees
		forest =  new decisionTree[treeNum];
		for(int i = 0; i<treeNum; i++) {
			forest[i] = new decisionTree(maxDepth, null, null);
			
			r = read.nextLine();//skip empty line
			forest[i].readTree(read);//read tree and reconstruct it
		}
	}
	
	//return a random wine in the data set
	public wine getWine() {
		Random r = new Random();
